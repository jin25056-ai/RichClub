...
def build_dataset_by_rules(df):
    ...
    # 스토캐스틱 데드크로스 (현재 역배열인데 직전은 정배열) -> 매도 조건
    ...
    # 4) OBV 및 OBV 시그널(9일) 계산
    direction = np.sign(df['close'].diff())
    direction.iloc[0] = 0
    df['obv'] = (direction * df['volume']).cumsum()
    df['obv_signal'] = df['obv'].rolling(window=9).mean()

    # 5) AI 모델용 파생 Feature 생성 (학습용 파생 변수에 거래량 관련 추가)
    df['ma5_20_ratio'] = df['ma5'] / df['ma20']
    df['ma20_60_ratio'] = df['ma20'] / df['ma60']
    df['close_ma60_ratio'] = df['close'] / df['ma60']
    df['vol_change'] = df['volume'].pct_change()
    df['vol_ma_ratio'] = df['volume'] / df['v_ma5']  # ⚡ 추가된 피처

    # ===== [당일 기준 기본 조건 정의] =====
    # 1. 양봉 조건 (종가 > 시가)
    cond_yang_bong = df['close'] > df['open']

    # 2. MACD 정배열 (0선 상관없이 MACD > Signal)
    cond_macd_pure = db_macd > db_macd_signal

    # 3. 스토캐스틱 정배열 (K > D)
    cond_stoch_pure = df['stoch_k'] > df['stoch_d']

    # 4. 5일-20일 조건: 골든크로스 당일 OR (5일선 > 20일선 정배열 상태이면서 동시에 5일 신고가 발생)
    ma5_20_prev_ratio = df['ma5'].shift(1) / df['ma20'].shift(1)
    cond_ma5_20_gc = (df['ma5'] > df['ma20']) & (ma5_20_prev_ratio <= 1)
    cond_ma5_20_pure_high = (df['ma5'] > df['ma20']) & df['is_5d_high']
    cond_ma_trend = cond_ma5_20_gc | cond_ma5_20_pure_high

    # 5. 60일선 조건: 60일 이평선 위에 이미 있거나 OR 아래에서 종가 기준 상향 돌파할 때
    cond_ma60_pure_above = df['close'] >= df['ma60']
    close_ma60_prev_ratio = df['close'].shift(1) / df['ma60'].shift(1)
    cond_ma60_golden_cross = (df['close'] >= df['ma60']) & (close_ma60_prev_ratio < 1)
    cond_ma60_filter = cond_ma60_pure_above | cond_ma60_golden_cross

    # 6. OBV 조건: 9일 시그널선 기준 골든크로스 OR 정배열
    obv_prev = df['obv'].shift(1)
    obv_sig_prev = df['obv_signal'].shift(1)
    cond_obv_gc = (df['obv'] > df['obv_signal']) & (obv_prev <= obv_sig_prev)
    cond_obv_pure = df['obv'] > df['obv_signal']
    cond_obv = cond_obv_gc | cond_obv_pure

    # ⚡ [신규 추가] 거래량 매수 필터 조건 (당일 거래량이 5일 거래량 이평선 이상이거나 상향돌파)
    v_ma5_prev_ratio = df['volume'].shift(1) / df['v_ma5'].shift(1)
    cond_vol_above = df['volume'] >= df['v_ma5']
    cond_vol_gc = (df['volume'] >= df['v_ma5']) & (v_ma5_prev_ratio < 1)
    cond_volume_filter = cond_vol_above | cond_vol_gc

    # ===== [관망 및 매수 예외 조건 정의] =====
    # ⚡ [관망 조건 수정] 5일선 상승 AND MACD 0선 위 AND 스토캐스틱 K선 상승 AND 5일 거래량 이평선도 상승세
    watch_signal = (df['ma5'] > df['ma5'].shift(1)) & \
                   (db_macd > 0) & \
                   (df['stoch_k'] > df['stoch_k'].shift(1)) & \
                   (df['v_ma5'] > df['v_ma5'].shift(1))

    # [매수 예외 조건] 강화된 관망 조건 속에서 (5-20골든크로스 발생) OR (60일선 종가 상향돌파 발생)
    watch_buy_exception = watch_signal & (cond_ma5_20_gc | cond_ma60_golden_cross)

    # ===== [레이블링 규칙 적용] =====
    df['target'] = -1

    # 기본 매수 전략 세트 (1일 전 시점 확인용) -> ⚡ 거래량 조건(cond_volume_filter) 결합됨
    base_buy_signal = cond_ma60_filter & cond_macd_pure & cond_stoch_pure & cond_ma_trend & cond_obv & cond_yang_bong & cond_volume_filter

    # [최종 매수 조건 결합] 1일전 모든 조건 충족 OR 당일 관망 예외 매수 조건 발생
    buy_mask = (base_buy_signal.shift(1) == True) | watch_buy_exception
    df.loc[buy_mask, 'target'] = 1

    # 2. 매도 조건 충족 -> 0 (매도) - 스토캐스틱 slow 데드크로스
    df.loc[is_stoch_dead_cross, 'target'] = 0

    # 3. 관망 조건 충족 -> 2 (관망)
    watch_mask = (df['target'] == -1) & watch_signal
    df.loc[watch_mask, 'target'] = 2

    # 데이터 정제 필터 (60일선 조건 유효 라인 외 배제)
    df.loc[(~cond_ma60_filter) & (df['target'] != 1), 'target'] = -1

    # 무한대 치환 및 결측치 제거 + 볼 가치 없는 데이터(-1) 제거
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    df = df[df['target'] != -1].copy()

    df.dropna(subset=['target', 'ma5_20_ratio', 'vol_change', 'stoch_k', 'obv', 'vol_ma_ratio'], inplace=True)
...
# 학습에 사용할 Feature 목록 (⚡ 거래량 지표인 vol_ma_ratio 추가 반영)
feature_cols = ['ma5_20_ratio', 'ma20_60_ratio', 'close_ma60_ratio', 'vol_change', 'vol_ma_ratio', 'macd', 'rsi',
                'stoch_k', 'stoch_d', 'obv']
X = dataset[feature_cols]
y = dataset['target']

if len(X) < 10:
    print(f"❌ 조건에 맞는 데이터가 너무 부족합니다 (현재: {len(X)}개).")
else:
    # ===== 4. 데이터 분할 =====
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=False)

    # ===== 5. 데이터 불균형 방지를 위한 클래스 가중치 계산 =====
    ...
    print("-" * 50)
    print(f"🎯 거래량(Volume MA) 조건이 추가된 모델 정확도(Accuracy): {accuracy_score(y_test, y_pred):.4f}")
    print("-" * 50)
    print("[상세 분류 레포트]")
    ...
    plt.title('거래량 필터 추가 후 혼동 행렬 (Confusion Matrix)', fontsize=14, pad=15)
