...
const AuthContainer: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'signup' && !form.name?.trim()) { setError('이름을 입력해주세요.'); return; }
    setLoading(true);
    try {
      ...
      <div style={styles.card}>
        <div style={styles.tabRow}>
          <button style={mode === 'login' ? styles.tabActive : styles.tab}
            onClick={() => { setMode('login'); setError(null); }}>로그인</button>
        ...
        <div style={styles.form}>
          ...
          <input style={styles.input} type="email" name="email" placeholder="이메일"
...
const styles: Record<string, React.CSSProperties> = {
  ...
  card: { width: '360px', border: '1px solid #2a2a2a', borderRadius: '10px', padding: '32px', backgroundColor: '#1a1a1a', display: 'flex', flexDirection: 'column', gap: '20px' },
  ...
  error: { fontSize: '13px', color: '#ff4d4d' },
