...
def main():
    ...
    for year in YEARS:
        ...
        # stk_cd, itmsNm 은 문자열로 먼저 읽기
        df = pd.read_csv(csv_path, encoding="cp949", dtype={"stk_cd": str, "itmsNm": str})
        logger.info(f"[{year}] {len(df)}행 로드 완료")

        # itmsNm, stk_cd 원본 보존 (피처 수치 변환 전에 미리 뽑아두기)
        raw_names = df["itmsNm"].tolist() if "itmsNm" in df.columns else []
        raw_codes = df["stk_cd"].tolist() if "stk_cd" in df.columns else []
        ...
        for idx, (_, row) in enumerate(df.iterrows()):
            try:
                predicted_at = datetime.strptime(str(row["date"]).split(".")[0], "%Y%m%d")
            except Exception:
                skipped += 1
                continue

            stock_code = raw_codes[idx].split(".")[0].zfill(6) if idx < len(raw_codes) else "000000"
            stock_name = raw_names[idx].strip() if idx < len(raw_names) else stock_code

            if idx < 3:
                logger.info(f"  sample[{idx}] -> code: {stock_code}, name: {stock_name}")
            ...
            doc = {
                ...
                "open":  _safe(row.get("mkp")),
                "high":  _safe(row.get("hipr")),
                "low":   _safe(row.get("lopr")),
